...
/*************************************************************************
 * 	LAB 15a - Arrays of Sheep
 * -----------------------------------------------------------------------
 * This programm will implement a sheep class and let the user choose what
 * to do with that information. The program will prompt the user to choose
 * an option of 0-6 and accordingly perform certain actions on the array
 * the user has created. The user may enter 0 to exit and the program will
 * repeat this process until the user enters 0.
 * -----------------------------------------------------------------------
 * INPUT:
...
int main()
{
	...
	SheepMenu menuChoice; // PROC		- an enum menu choice
 ...
 **************************************************************************/
	MenuOptions();
	userInput = GetAndCheckIntInp("Enter selection: ", 0, 6);
	menuChoice = ConvertIntToEnumType(userInput);
	cout << endl;

/**************************************************************************
 * PROC - Program will enter a while loop based on the user input.
	...
	while(menuChoice != EXIT)
	{
		...
		switch (menuChoice)
		{
			...
			case OUTPUTFIRST:
					...
					//Displays the info of one sheep if the list is full
					if(firstSheep.GetName() != " ")
					{
			...
			case FINDSHEEP:
					...
					else
					{
					 ...
					 break;
			case LISTSIZE:
				   totalSheep = sheepList.TotalSheep();
